...
| 42-0 | 72 | `PATCH /api/user/mypage/password/reset` | 비밀번호 변경 (현재 비밀번호 재확인 — 변경 후 재로그인 확인) | ☐ |
...
- [ ] 회원가입: 인증번호 검증 없이 바로 register 호출 → **403 "이메일 인증이 완료되지 않았습니다."**
- [ ] 비밀번호 초기화(84): 인증 검증 없이 바로 호출 → **401** (기존엔 아이디만 알면 변경 가능했던 구멍)
- [ ] 비밀번호 변경(72): 현재 비밀번호 틀리게 → 400 / 규칙 미달(예: 1234) → 400 / 성공 후 새 비번으로 재로그인
