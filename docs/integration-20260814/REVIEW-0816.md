...
| 웹 푸시 (2기 확정) | **구현 완료.** "구독" 용어 배제 — 테이블 `notification_devices`(알림 수신 기기 등록부), API 는 toast 네임스페이스: `PUT /api/user/mypage/toast/devices`(수신 동의/거부 통합), `GET .../toast/devices`(수신 동의 상태), `GET .../toast/vapid-key` (명세 id 136·137·138). 등록·해제를 PUT 하나로 합친 이유는 프론트가 서버 상태를 몰라도 원하는 상태만 보내면 되게 하기 위함이고, 토글 초기값 판정용으로 GET 을 추가했다(브라우저 구독만 믿으면 서버가 404/410 기기를 정리한 뒤 "토글 켜짐인데 알림 안 옴" 상태를 감지 못한다). 알림 저장 직후 등록 기기로 @Async 발송(NotificationPushService), 404/410 기기 자동 정리. 라이브러리 nl.martijndwars:web-push 5.1.1, VAPID 키는 application.properties |
...
| 회원가입 형식 검증 (추가) | 백엔드에 형식 검증이 전혀 없어 프론트 zod 규칙(schemas/auth.js)을 API 직접 호출로 우회 가능했음. 서버 검증 추가 — 규칙·문구 프론트와 동일, 정규식은 policy_settings(signup_*_regex) 6행. 이름·아이디 50자/이메일 150자 상한은 DB 컬럼 가드로 백엔드만 추가(프론트에 max 없음 — 프론트 보완 권장) |
| 알림 단건 읽음/삭제 응답 확정 (PM 결정 2026-08-17) | `{message}` 만 반환하던 설계가 빈약하다는 지적 — 사용자는 알림을 읽기만 하지 않고 **대상으로 이동**한다. 단건 읽음 응답에 이동 정보(type/targetType/targetId/boardId/linkUrl) + unreadCount 를 싣기로 확정. **이미 읽은 알림도 200 멱등**(재클릭 이동 보장). 삭제는 notificationId + unreadCount 만. `linkUrl` 은 API 경로이며 화면 경로가 아님 — 게시판이 데이터 정의라 백엔드가 프론트 라우팅을 알 수 없으므로 조립은 프론트 책임. 목록(id=25)에도 같은 이유로 boardId 추가. 명세 25·26·27 반영, 지시서에 '임의 변경 금지'로 명시 |
| 알림 롤백 (PM 지시) | 알림함 API 5종 + **발행 로직**(NotificationService.publish/notifyXxx, insertNotification, BoardServiceImpl 훅)까지 제거하고 담당자 과제로 환원. 웹 푸시(VAPID·만료 기기 자동정리)·기기 등록부만 유지(난이도 높은 영역은 유지한다는 PM 방침). 확정 정책: 목록은 페이징 없이 전체 반환 + 최근 2개월(policy_settings.notification_list_months). 제거분은 **git 브랜치 `archive/notification-crud`(fc63b9b)** 에 보관 — **지시서와 코드 주석에는 일부러 적지 않음**(정답 노출 방지). 담당자가 스스로 설계하도록 지시서는 요구사항만 제시 |
| 일정 카테고리 재적용 (PM 결정) | `event_categories` 테이블·시드만 PM 이 적용하고 API 는 담당자 과제로. 지시서 `HANDOFF-event-category-tasks.md` — **보관 브랜치 `archive/event-categories`(da6da1d)는 지시서·백로그에 미기재**(정답 노출 방지, PM 문서에만 기록) |
| 아동 안전 표준 페이지 (추가) | Google Play 제출용 child-safety.html. 신고 사유에 `CHILD_SAFETY`(아동 안전 위반 · 아동 성착취물) 1행 추가(display_order 4, ADULT 뒤). 연령 표기는 방침·아동안전 두 문서 모두 **성인 대상 폐쇄형 + 가입 후 학번 대조 강제 탈퇴**로 통일 — 생년월일을 수집하지 않으므로 '18세 미만 차단' 같은 미구현 통제는 기재하지 않음 |
| 강제 탈퇴 권한 상향 (PM 결정) | forceWithdraw 가 `requireAdmin()`(Lv1~) 이라 최하위 관리자도 되돌릴 수 없는 개인정보 파기를 실행할 수 있었음 → **`requireAdminLevel(3)`** 로 상향. 다른 관리 조치는 전부 소프트삭제+moderation_log 라 복구 가능하므로 Lv1 유지. Play 심사팀에 Lv1 관리자 계정을 제공하는 전제와도 맞물림 |
| Play 심사 계정 2종 (추가) | 관리자 화면이 이번 릴리스에 포함되므로 앱 액세스에 일반(`testtest`)·관리자(`testadmin`, Lv1) 두 계정 등록. 모든 `/api/admin/**` 이 `requireAdmin()` 이라 Lv1 도 관리 화면 전체를 볼 수 있고, Lv3 과의 차이는 write_level 2~3 게시판 작성뿐 |
| 명예의전당 탈퇴 정책 (확정) | donations.display_name 은 users 조인이 아닌 등재 시 스냅샷 — 탈퇴해도 이름 유지. PM 결정(2026-08-16): **유지 + 본인 요청 시 이메일로 삭제·익명 처리**. privacy-policy.html §6·account-deletion.html 데이터 표에 명시. 코드 변경 없음 |
| 비밀번호 초기화 형식 검증 (추가) | resetPassword(id=84, 1기)는 새 비밀번호 형식 검증이 없어 초기화 경로로 규칙 위반 비밀번호 설정이 가능했음 → 2026-08-16 PM 지시로 백엔드 검증 추가(회원가입과 동일 규칙 signup_password_regex 재사용, 400) |
