...
| 알림함 API 롤백 (PM 지시) | 조회·읽음(단건/전체)·삭제·미읽음수 5종을 제거하고 담당자 과제로 환원. 발행 훅·웹 푸시·기기 등록부는 유지(난이도 높은 영역은 유지한다는 PM 방침). 제거분은 **git 브랜치 `archive/notification-crud`(fc63b9b)** 에 보관 — **지시서와 코드 주석에는 일부러 적지 않음**(정답 노출 방지). 담당자가 스스로 설계하도록 지시서는 요구사항만 제시 |
