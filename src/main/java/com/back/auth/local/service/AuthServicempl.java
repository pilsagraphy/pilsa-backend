...
import com.back.auth.core.dto.*;
import com.back.auth.local.dto.*;
...
import com.back.auth.local.service.MailService;
...
public class AuthServicempl implements AuthService {
    ...
    /**
     * 세션을 여는 일(제재 확인·토큰 발급·리프레시 쿠키)은 전부 여기에 맡긴다.
     * 소셜 로그인도 같은 컴포넌트를 쓰므로 인증 수단이 늘어도 검사가 갈라지지 않는다.
     */
    private final TokenIssuer tokenIssuer;

    // 아래 네 메서드는 TokenIssuer 로 옮겼다. 호출 지점이 많아 이름만 남겨 위임한다.

    private void addRefreshTokenCookie(HttpServletResponse response, String refreshToken, boolean autoLogin) {
        tokenIssuer.addRefreshTokenCookie(response, refreshToken, autoLogin);
    }

    private int autoLoginDays() {
        return tokenIssuer.autoLoginDays();
    }

    private void checkNotBanned(UserDto user) {
        tokenIssuer.checkNotBanned(user);
    }

    private void checkTokenVersion(Claims claims, UserDto user) {
        tokenIssuer.checkTokenVersion(claims, user);
    }
    ...
    public AuthResponse login(LoginRequest request,
        ...
        // 토큰 발급·리프레시 쿠키는 소셜 로그인과 같은 경로를 탄다
        AuthResponse issued = tokenIssuer.issueFor(user, autoLogin, response);
    ...
    public void logout(HttpServletResponse response, HttpServletRequest request) {
        ...
        tokenIssuer.clearRefreshTokenCookie(response);
...
//            }
     ...
     * 모든 기기에서 로그아웃.
     *
     * users.token_version 을 올리면 그 사용자 앞으로 발급된 액세스·리프레시 토큰이 전부 무효가 된다
     * (JwtAuthenticationFilter 와 extend/refresh 가 대조한다). 다른 기기는 다음 요청에서 401 을 받는다.
     * 무상태 JWT 라 이 방법 외에는 발급된 토큰을 취소할 수단이 없다 — 자세한 배경은 JwtUtil 참고.
     */
    @Override
    @Transactional
    public void logoutAllDevices(HttpServletRequest request, HttpServletResponse response) {
        Long userId = AuthUtils.currentUserId();
        authMapper.bumpTokenVersion(userId);
        log.info("모든 기기에서 로그아웃 - userId: {}", userId);

        // 지금 이 기기의 refreshToken 쿠키도 함께 만료시킨다 (안 그러면 쿠키만 남아 재발급을 시도하다 401 이 뜬다)
