...
public class AuthController {
    ...
    @Operation(
            ...
            description = """
                    ...
                    실패: 409 {"message":"이미 가입된 학번입니다."}
                    실패: 409 {"message":"이미 등록된 전화번호입니다."}
                    실패: 400 {"message":"학번은 필수입니다."}
                    실패: 400 {"message":"유효하지 않은 회원 구분입니다. (STUDENT/ALUMNI)"}
                    실패: 403 {"message":"가입이 제한된 학번입니다."} — 영구차단 상태로 탈퇴한 학번
                    실패: 403 {"message":"가입이 제한된 학번입니다. (YYYY-MM-DD 이후 가입 가능)"} — 정지 중 탈퇴한 학번

                    ※ 재가입 대조: 탈퇴 시 학번은 복원 불가능한 해시로 보관되며(부정 이용 방지),
                    제재 이력 없는 일반 탈퇴자는 같은 학번으로 자유롭게 재가입할 수 있다.
                    """
    )
