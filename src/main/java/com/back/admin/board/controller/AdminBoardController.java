...
public class AdminBoardController {
    ...
    @Operation(summary = "새 게시판 생성 (관리자)",
            description = """
                    ...
                    새 게시판은 목록 맨 뒤에 붙는다(displayOrder 는 서버가 정한다 — 순서 변경은 PATCH 로).

                    이름 중복은 **살아있는 게시판끼리만** 따진다. 삭제된 게시판이 쓰던 이름은 다시 쓸 수 있으며,
                    그렇게 만든 게시판은 새 boardId 를 받는 **별개의 게시판**이다(옛 게시판의 글·신고·제재 이력은 그대로 남는다).
    ...
    @Operation(summary = "게시판 수정 (관리자)",
            description = """
                    ...
                    ### 노출 순서 변경 (드래그앤드롭)
                    `displayOrder` 는 **옮겨 놓은 자리가 몇 번째인지**(1부터)를 뜻한다. 서버가 나머지 게시판을
                    한 칸씩 밀어 전체 순번을 1..N 으로 다시 채우므로, 프론트는 **끌어놓은 게시판 하나만** 보내면 된다.

                    ```json
                    {"displayOrder": 3}     // 이 게시판을 3번째 자리로. 사이에 낀 게시판들이 자동으로 밀린다
                    ```
                    범위를 벗어난 값(0 이하, N 초과)은 각각 처음·끝으로 맞춘다.
                    삭제·생성 시에도 순번은 항상 1..N 으로 유지되므로 번호에 구멍이 생기지 않는다.
    ...
    @Operation(summary = "게시판 삭제 (관리자, 소프트)",
            description = """
                    ...
                    삭제된 게시판은 이름을 놓아주므로 같은 이름으로 새 게시판을 만들 수 있다(새 boardId 의 별개 게시판).
                    남은 게시판의 노출 순서는 자동으로 1..N 으로 다시 채워져 번호에 구멍이 남지 않는다.
